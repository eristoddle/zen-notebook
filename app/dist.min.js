...
Showdown.converter = function (a) {
    ...
    var runUpdate = function () {
        var copyPath, execPath, gui = require("nw.gui"), pkg = require("./package.json"), updater = require("node-webkit-updater"), upd = new updater(pkg);
        gui.App.argv.length ? (copyPath = gui.App.argv[0], execPath = gui.App.argv[1], upd.install(copyPath, function (err) {
            err || (upd.run(execPath, null), gui.App.quit())
        })) : upd.checkNewVersion(function (error, newVersionExists, manifest) {
            console.log(newVersionExists), !error && newVersionExists && upd.download(function (error, filename) {
                error || (console.log("downloading"), upd.unpack(filename, function (error, newAppPath) {
                    error || (console.log("running installer"), upd.runInstaller(newAppPath, [upd.getAppPath(), upd.getAppExec()], {}), gui.App.quit())
                }, manifest))
            }, manifest)
        })
    };
...
var isNodeWebkit = "object" == typeof process, platformModule = null, win, mute = !1;
...
var zenNotebook = angular.module("zenNotebook", ["ngSanitize", platformModule]).run(function ($rootScope, storageFactory) {
    $rootScope.mute = mute, storageFactory.getStorage("mute") && ($rootScope.mute = storageFactory.getStorage("mute")), $rootScope.active_component = storageFactory.getStorage("active_component"), $rootScope.active_component || ($rootScope.active_component = "notebook")
});
zenNotebook.filter("object2Array", function () {
    ...
    var factory = $injector.get($rootScope.active_component + "Factory"), component_nav = factory.getMenu(), app_nav = [
    ...
    return{message: null, menus: {nav: nav}, publishClick: function (message) {
        this.message = message, "left" == message.sub && $rootScope.$broadcast("toggleLeft"), "foot" == message.sub && $rootScope.$broadcast("toggleFoot"), "body" == message.sub && $rootScope.$broadcast("body"), "nw" == message.sub && ("maximize" == message.action && win.toggleFullscreen(), "minimize" == message.action && win.minimize(), "exit" == message.action && (factory.onExit(), win.close()))
    ...
    $scope.foot = {}, $scope.components = ["notebook", "nanowrimo"], $scope.$on("toggleFoot", function () {
        ...
        $scope.footChangeClass = !$scope.footChangeClass, $scope.foot.partial = "footer/" + message.action + ".html", $scope.expr = function (locals) {
            ...
            $rootScope.active_component = component, storageFactory.setStorage("active_component", component), $rootScope.$broadcast("loadComponent")
        }
    })
    ...
    var notebook = {years: {}, file: null, currentDate: null, activeDate: null, activeDateText: function () {
        ...
        window.localStorage && window.localStorage.setItem("content", content), window.localStorage && window.localStorage.setItem("word_count", count)
        ...
        try {
            ...
            return window.localStorage && window.localStorage.setItem("error", err), ""
        }
        ...
        return window.localStorage.getItem("content").replace(/<br>/g, "\n").replace(/<div>/g, "\n").replace(/<\/div>/g, "")
    ...
    }, activeTags: "", getStorage: function (key) {
        ...
        try {
            return fileDialog.writeFile(filename, journal), window.localStorage && window.localStorage.setItem("file", this.file), journal
        } catch (err) {
            window.localStorage && window.localStorage.setItem("error", err), window.localStorage && window.localStorage.setItem("recovery", journal)
        }
    }, loadNotebook: function (file) {
        var data = fileDialog.readFile(file);
        tempJournal = JSON.parse(data), this.file = file, this.currentDate = new Date, this.years = tempJournal.years, window.localStorage && window.localStorage.setItem("file", this.file)
    }};
    ...
    return{documents: {}, file: null, title: null, currentChapter: null, startDate: null, goalDate: null, goalWords: null, currentDate: null, currentWords: null, chapterCount: function () {
        ...
        var data = fileDialog.readFile(file);
        tempBook = JSON.parse(data), this.file = file, storageFactory.setStorage("file", this.file, "nanowrimo"), this.documents = tempBook.documents
    }}
}]);